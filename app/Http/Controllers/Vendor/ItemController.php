...
class ItemController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if($request->item_id && $request?->product_gellary == 1 ){
            ...
            if(!$request->has('image')){
                $oldPath = storage_path("product/{$item_data->image}");
                $newFileName =\Carbon\Carbon::now()->toDateString() . "-" . uniqid() . ".png" ;
                $newPath = storage_path("product/{$newFileName}");
            ...
            foreach($uniqueValues as$key=> $value){
                $oldPath = storage_path("product/{$value}");
                $newFileName =\Carbon\Carbon::now()->toDateString() . "-" . uniqid() . ".png" ;
                $newPath = storage_path("product/{$newFileName}");
                if (File::exists($oldPath)) {
                    File::copy($oldPath, $newPath);
    ...
    public function list(Request $request)
    {
        ...
        return view('vendor-views.product.list', compact('items', 'category', 'type','sub_categories', 'toggle_veg_non_veg'));
    }
    ...
    public function store_temp_data($data, $request,$tag_ids , $update =null)
    {
        ...
        if($request->has('image')){
            ...
            $oldPath = storage_path("product/{$data->image}");
            $temp_image_name =\Carbon\Carbon::now()->toDateString() . "-" . uniqid() . ".png" ;
            $newPath = storage_path("product/{$temp_image_name}");
            if (File::exists($oldPath)) {
                File::copy($oldPath, $newPath);
            }
            $temp_item->image = $temp_image_name;
