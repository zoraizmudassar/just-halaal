...
class DeliverymanController extends Controller
{
    ...
    public function update_order_status(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'customer_identity_proof' =>'array|max:5',
        ]);
        ...
        if ($request->status == 'delivered')
        {
            ...
            $img_names1 = [];
            $images1 = [];
            if (!empty($request->file('customer_identity_proof'))) {
                foreach ($request->customer_identity_proof as $img) {
                    $image_name1 = Helpers::upload('order/', 'png', $img);
                    array_push($img_names1, $image_name1);
                }
                $images1 = $img_names1;
            }
            $order->order_proof = count($images)>0?json_encode($images):'';
            $order->customer_identity_proof = count($images1)>0?json_encode($images1):'';
